...
- 교육용 데모 클라이언트/Android 앱이 시나리오를 실행
...
- REST 테스트 엔드포인트 제공(VC/VP 생성/검증 포함)
- Swagger 기반 문서화
- 데모 플로우용 API(신원 VC 발급/렌터카 계약 발급/챌린지) 제공
...
### 3.5 REST 데모 플로우(신원 VC → 계약 VC → 접근 제어)
1) Holder DID 발급
2) 신원 VC 발급: Simple Presentation(VC 없는 VP) 검증 후 면허 VC 발급
3) 렌터카 계약: VP 서명 + aud/nonce + 면허 VC 무결성/만료 검증 후 계약 VC 발급
4) 차량 접근: 계약 VC로 VP 생성 → 무결성 검증 후 접근 허용/거부
...
### 2.8 Android 데모 앱 (android/)
- did_service_endpoint REST API를 호출하여 시나리오를 실행
- JNI(c-shared)로 키 생성/서명을 수행
