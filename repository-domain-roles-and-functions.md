...
- 핵심 구성: `did` 라이브러리(암호/문서/드라이버), `did-registry`(저장소), `did-sep`(리졸버), `did_service_endpoint`(REST 게이트웨이), `greeter_*` 데모 클라이언트·릴라잉파티·발급자, `geth_client`(체인 연동 예제), `proto-files`(gRPC 스키마).
